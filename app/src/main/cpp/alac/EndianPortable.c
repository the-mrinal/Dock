...
#define TARGET_RT_LITTLE_ENDIAN 1
// Apple's reference source only covered x86 here; on Android the NDK targets
// ARM (aarch64, armv7) and x86 (32/64) — all little-endian under the standard
// AAPCS ABI. Without this branch Swap32BtoN was a no-op on ARM, parsing the
// magic cookie's big-endian `frameLength=352` as `0x60010000` and blowing
// out calloc inside ALACDecoder::Init (kALAC_MemFullError = -108).
#elif defined(__ARMEL__) || defined(__aarch64__) || defined(__arm__) || \
